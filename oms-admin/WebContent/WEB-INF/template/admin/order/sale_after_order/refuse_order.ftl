...
<title>创建拒收订单 - Powered By ${systemConfig.systemName}</title>
...
$().ready( function() {
	 $("#refuse").click( function() {
		...
		if(!checkDetail()){
     	...
     	// defind in refuse.order.view.js  校验退款信息
...
<body class="input">
	<div class="body">
		<div class="inputBar">
			<h1><span class="icon">&nbsp;</span>创建拒收订单</h1>
		</div>
		<form id="inputForm" class="validate" action="refuse!addRefuse.action" method="post">
			...
			<div id="accordion">
			  <h2>拒收订单信息<#if order.hadCreateRefuse>(<font color="red">已拒收</font>)</#if></h2>
			  <#include "/WEB-INF/template/admin/order/sale_after_order/detail_information_page.ftl">
				
			</div>
			
  			
		    <#--include "/WEB-INF/template/admin/order/sale_after_order/refuse_sub_order.ftl"-->
		    <#include "/WEB-INF/template/admin/order/sale_after_order/sub_order.ftl">
							
			<div class="buttonArea">
				<input type="button" class="formButton" value="确认拒收" hidefocus="true"  id="refuse" <#if order.hadCreateRefuse>disabled style="color:gray;"</#if>/>&nbsp;&nbsp;&nbsp;&nbsp;
