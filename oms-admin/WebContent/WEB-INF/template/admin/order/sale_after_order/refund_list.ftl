...
//      if($(this).attr("checked")) 
      if(this.checked)
	  {   
	   	if($("#statusTotalCode_"+index).val() == "0231")
		{
			conOrder += $(this).val() + ",";
			confirmed = true;
		}
...
//      if($(this).attr("checked"))
	...
	if(yundianNoNeedConfirmed)
	{
		alert("逆向订单" + yundianNotConOrder.substring(0,yundianNotConOrder.length-1) + "是云店订单，需在云店管理系统审核！");
		return;
	}
...
function orderCancel(){
	...
	$("[name='ids']").each(function(){
      if($(this).attr("checked"))     
	  {  
	   	if($("#statusTotalCode_"+index2).val() != "0220" && $("#statusConfirmCode_"+index2).val() != "0807")
		{
	...
	if(noNeedConfirmed)
	{
		alert("订单" + notCanOrder.substring(0,notCanOrder.length-1) + "不满足取消条件");
		return;
	}
			...
			<table class="listTable" style="border: 1px solid #CCCCCC;width: 100%;">
				...
				<tr>
					<td colspan="4">
						...
						<input type="button" class="button" <#if (pager.list?size <= 0)>disabled</#if> name="storeReceive" id="storeReceive" onclick="orderReceive();" value="门店已收货">
						<input type="button" class="button" <#if (pager.list?size <= 0)>disabled</#if> name="storeRefund" id="storeRefund" onclick="orderRefund();" value="门店已退款">
				...
				<#list pager.list as list>
					<tr ondblclick="window.open('refund!view.action?orderNo=${list.orderNo}&column=${column}')">
						...
						<td>
							...
							<#if (list.statusTotal == '0241') || (list.statusConfirm == '0808')>
							<br/>
