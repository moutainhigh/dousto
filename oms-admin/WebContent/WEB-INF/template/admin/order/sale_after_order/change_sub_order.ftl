				...
				<#list orderSub.orderItems as list>
				  <#if (list.remainNum > 0)>	
					<tr name="orderitemlist">
					   ...
					   <td><input type="checkbox" name="ids" value="${list_index}" />
						...
						<td>
							${list.skuNo!}
							<input type="hidden" name="orderItem.skuNo"  id="skuNo_${list_index}"  value="${list.skuNo!}" />
						</td>
						<td>
					...
					<#else>
					  	<tr>
					  		<td colspan="13" style="text-align:center;color:red;">无可换商品！</td>
					  	</tr>
					</#if>
				</#if>
			</table>
