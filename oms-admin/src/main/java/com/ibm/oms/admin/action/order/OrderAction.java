...
public class OrderAction extends AbstractOrderAction {
	...
	public String updateSubDetail() {
		...
		if(morder1.getClientServiceRemarkFlag() != order.getClientServiceRemarkFlag()){
			content.append("客服备注等级：</br>");
			oldData.append(morder1.getClientServiceRemarkFlag()).append("</br>");
			newData.append(order.getClientServiceRemarkFlag()).append("</br>");
		}
		
		morder1.setClientServiceRemark(order.getClientServiceRemark());
		morder1.setDeliveryDateFlag(order.getDeliveryDateFlag());
		morder1.setClientServiceRemarkFlag(order.getClientServiceRemarkFlag());
	...
	public String batchCancelOrder() {
		...
		for (String orderSubNo : orderSubNos) {
			...
			log.setReason("取消订单");
			orderOperateLogService.save(log);    
			OrderMain newOrderMain = orderMainService.findByOrderNo(order.getOrderNo());
...
//			boolean isCancel = true;//【saveCancelOrder中实现】
            ...
            try {
                ...
                ResultDTO resultDTO = saleAfterOrderService.cancelOrderByService(orderSubNo, cancelScene);
                if (resultDTO.getResult() == -1) {
...
//                sb.append("订单审核失败!"+auditResult);
	...
	public String batchUpdateClientServiceRemark() {
		...
		Integer clientServiceRemarkFlag = Integer.valueOf(this.getIntegerParamter("clientServiceRemarkFlag"));
		...
		for (String orderSubNo : orderSubNos) {
			sb.append("订单号：" + orderSubNo);
			try {
				orderMain.setClientServiceRemark(clientServiceRemark);
				orderMain.setClientServiceRemarkFlag(clientServiceRemarkFlag);
				orderMain.setOrderNo(orderNoService.getOrderNoBySubNo(orderSubNo));
				orderMainService.updateClientServiceRemark(orderMain);
				sb.append("订单修改成功!");
			} catch (Exception e) {
				e.printStackTrace();
				sb.append("订单修改失败!");
    ...
    private Integer getIntegerParamter(String key){
    	return this.getParameter(key) == null ? 0 : Integer.valueOf(this.getParameter(key));
    }
