...
public class OrderSearchDaoImpl extends BaseDaoImpl<OrderSearch, Long>
	...
	private List<OrderSearch> convertOrderSearchSqlList(List<Object[]> resultList){
		...
		if(resultList!=null && !resultList.isEmpty()){
			for ( Object[] values : resultList ) {
			    ...
			    orderSearch.setPayAmount(isNull(values[57])?null:(BigDecimal)values[57]);
			    orderSearch.setPayModeName(isNull(values[58])?null:((String)values[58]));
			    orderSearch.setPayStatus(isNull(values[59])?null:((Integer)values[59]).longValue());
			    orderSearch.setSellerMessage(isNull(values[60])?null:((String)values[60]));
	...
	public String querySql(OrderSearch order,List<String> statusPayOther, List<String> statusTotalOther, int columnId) {
	   ...
	   sb.append("ordersearc0_4_.DATE_UPDATED             as col_55_0_,");
	   sb.append("ordersearc0_.CLIENT_REMARK              as col_56_0_,");
	   sb.append("ordersearc0_4_.PAY_AMOUNT               as col_57_0_,");
	   sb.append("ordersearc0_4_.PAY_MODE_NAME            as col_58_0_,");
	   sb.append("ordersearc0_4_.PAY_STATUS               as col_59_0_,");
	   sb.append("ordersearc0_.SELLER_MESSAGE             as col_60_0_");
	   ...
	   //增加2个判定条件 有支付日期时搜索
		   ...
		   sb.append(" left join ORDER_PAY_MODE  ordersearc0_4_  on ordersearc0_.id = ordersearc0_4_.ID_ORDER");
	   ...
	   if(StringUtils.isNotBlank(order.getStatusTotal())) {
 	   ...
 	   if(StringUtils.isNotBlank(order.getPayModeCode())){
 		   sb.append(" and ordersearc0_4_.PAY_MODE_CODE='").append(order.getPayModeCode()).append("'"); 
 	   }
	...
	// 逆向单不需查询“运费补款单”
	   ...
	   if(StringUtils.isNotBlank(order.getStatusConfirm())){
		...
		if (!(payTimeFrom == null) && !(payTimeTo == null)) {
			sb.append(" and ordersearc0_4_.DATE_UPDATED >= " + "str_to_date('" + DateUtils.formatGeneralDateTimeFormat(payTimeFrom) + "','%Y-%m-%d %H:%i:%s')");
			sb.append(" and ordersearc0_4_.DATE_UPDATED <= " + "str_to_date('" + DateUtils.formatGeneralDateTimeFormat(payTimeTo) + "','%Y-%m-%d %H:%i:%s')");
