	<div class="body">
		 <div id="tabs">
			<ul>
				...
				<#if order.orderCategory == 'ret' || (order.orderCategory == 'rej' && order.statusPay =='0420') || (isNeed == 1)>
					<!-- 退货、在线支付已支付拒收需退款 -->
	  				<li><a href="#retInfo">退款信息</a></li>
				</#if>
				<#if order.orderCategory != 'rej'> 
		   ...
		   <div id="orderItem" style="display:block">
			<table class="inputTable tabContent">
				<tr class="title">
					<th>订单明细号</th>
					<th>SKU</th>
					<#if order.orderCategory == 'ret'>
					<th>退货数量</th>
					<#elseif order.orderCategory == 'chg'>
					<th>换货数量</th>
					<#elseif order.orderCategory == 'rej'>
					<th>数量</th>
					</#if>
					<th>折前单价</th>
					<th>积分</th>
					...
					<tr name="orderitemlist">
						...
						<td>
							<#if isDetail || (order.statusConfirm == '0807')>
								...
								<input type="hidden"  style="width:60px;" name="orderRetChgItemsList[${list_index}].unitPrice" id="unitPrice_${list_index}"   value="${list.unitPrice?default(0)}" />
							<#else>
								<input type="text"  style="width:60px;" name="orderRetChgItemsList[${list_index}].unitPrice" id="unitPrice_${list_index}"  onblur="javascript:changeUnitPrice(${list_index})" class="formText {required: true, min: 0}" value="${list.unitPrice?default(0)}" />
						 	</#if>
						</td>
						<td>
							<#if isDetail || (order.statusConfirm == '0807')>
								${list.productPoint?default(0)}
								<input type="hidden"  style="width:60px;" name="orderRetChgItemsList[${list_index}].productPoint" id="productPoint_${list_index}"  value="${list.productPoint?default(0)}" />
							<#else>
						...
						<td>
							<#if isDetail || (order.statusConfirm == '0807')>
								...
								<input type="hidden"  style="width:60px;" name="orderRetChgItemsList[${list_index}].unitDiscount" id="unitDiscount_${list_index}"  value="${list.unitDiscount?default(0)}" />
							<#else>
						...
						<td>
							<#if isDetail || (order.statusConfirm == '0807')>
								...
								<input type="hidden"  style="width:60px;" name="orderRetChgItemsList[${list_index}].couponAmount" id="couponAmount_${list_index}"  value="${list.couponAmount?default(0)}" />
							<#else>
						...
						-->
						<td>
						  <select  id="preRefundReason_${list_index}" onchange="javascript:getRefundReasonFun(${list_index})" <#if isDetail || (order.statusConfirm == '0807')> disabled<#else>name="orderRetChgItemsList[${list_index}].preRefundReason"</#if>>
						...
						<td>
						  <select  id="refundReason_${list_index}" <#if isDetail || (order.statusConfirm == '0807')> disabled<#else>name="orderRetChgItemsList[${list_index}].reason"</#if>>
		   ...
		   <div id="customerInfo" style="display:none">
			...
			<#if order.orderCategory == 'chg'>
				<!-- 换货不需退款 -->
			<#elseif (order.orderCategory == 'rej' && order.statusPay =='0420') || (isNeed == 1)>
				<!-- 在线支付已支付拒收需退款 -->
