...
(function($) {
	...
	UploadHandler = function(container, options) {
		...
		this.templteTable = container.find('.template_table:first');
		this.downloadTable = this.uploadTable;
		this.progressAllNode = container
				.find('.file_upload_overall_progress div:first');
		this.uploadTemplate = this.templteTable
				.find('.file_upload_template:first');
		this.downloadTemplate = this.templteTable
				.find('.file_download_template:first');
		...
		this.enableDragToDesktop = function() {
			var link = $(this), url = link.get(0).href, name = decodeURIComponent(
					url.split('/').pop()).replace(/:/g, '-'), type = 'application/octet-stream';
		...
		this.buildMultiUploadRow = function(files, handler) {			
			...
			$
					.each(
							files,
							function(index, file) {
								var row = handler.buildUploadRow(files, index,
										handler).show(), cells = row
										.find('.file_upload_progress, .file_upload_start, .file_upload_cancel');
								if (index) {
									cells.remove();
								} else {
		...
		this.buildUploadRow = function(files, index, handler) {
			this.uploadTable.empty();
			if (typeof index !== 'number') {
				return handler.buildMultiUploadRow(files, handler);
			}
			var file = files[index], fileName = handler
					.formatFileName(file.name), uploadRow = handler.uploadTemplate
					.clone().removeAttr('id');
			uploadRow.find('.file_name').text(fileName);
			...
			if (handler.autoUpload) {
				...
				uploadRow.find('.file_upload_start button').button({
					icons : {
			...
			uploadRow.find('.file_upload_cancel button').button({
				icons : {
		...
		this.buildDownloadRow = function(file, handler) {
			...
			var fileName = handler.formatFileName(file.name), fileUrl = handler
					.getFileUrl(file, handler), thumbnailUrl = handler
					.getThumbnailUrl(file, handler), downloadRow = handler.downloadTemplate
					.clone().removeAttr('id');

			downloadRow.attr('data-id', file.id || file.name);
			downloadRow.find('.file_name a').text(fileName);
			downloadRow.find('.file_size').text(
					handler.formatFileSize(file.size));

//			alert(thumbnailUrl);
			
			$("#upload_image_callback").attr("call_back_value",thumbnailUrl);
			if (thumbnailUrl) {
				var rootPath = $("#resourceRootPath").val();
				downloadRow.find('.file_download_preview').append(
						$('<a/>').append(
								$('<img/>').attr('src', rootPath+thumbnailUrl )));
				downloadRow.find('a').attr('target', '_blank');
			}

			downloadRow.find('a').attr('href', fileUrl || null).each(
					handler.enableDragToDesktop);
			downloadRow.find('.file_download_delete button').button({
				icons : {
		...
		this.beforeSend = function(event, files, index, xhr, handler, callBack) {
			...
			if (!regexp.test(files[index].name)) {
				...
				// handler.uploadRow.find('.file_upload_progress').html('ONLY
				// IMAGES ALLOWED!');
				// setTimeout(function () {
				// handler.removeNode(handler.uploadRow);
				// }, 10000);
			...
			if (handler.autoUpload) {
				...
				handler.uploadRow.find('.file_upload_start button').click(
						function(e) {
							$(this).fadeOut();
							callBack();
							e.preventDefault();
		...
		this.downloadHandler = function(e) {
			if (uploadHandler.forceIframeDownload) {
				// Open download dialogs via iframes, to prevent aborting
				// current uploads:
				$('<iframe style="display:none;"/>').attr('src', this.href)
						.appendTo(container);
				e.preventDefault();
			}
		};
		...
		this.deleteHandler = function(e) {
			...
			$.ajax({
				url : uploadHandler.url + '?operate=delete&file='
						+ encodeURIComponent(row.attr('data-id')),
				type : 'DELETE',
				success : function() {
		...
		this.initEventHandlers = function() {
			uploadHandler.downloadTable.find('a:not([target="_blank"])').live(
					'click', uploadHandler.downloadHandler);
		...
		this.destroyEventHandlers = function() {
			uploadHandler.downloadTable.find('a:not([target="_blank"])').die(
					'click', uploadHandler.downloadHandler);
			uploadHandler.downloadTable.find('.file_download_delete button')
		...
		this.multiButtonHandler = function(e) {
			uploadHandler.uploadTable.find(e.data.selector + ' button:visible')
					.click();
			e.preventDefault();
		};
		...
		this.initMultiButtons = function() {
			if (uploadHandler.autoUpload) {
				...
				uploadHandler.multiButtons.find('.file_upload_start:first')
						...
						}, uploadHandler.multiButtonHandler);
			}
	...
	methods = {
		...
		option : function(option, value, namespace) {
			if (!option
					|| (typeof option === 'string' && typeof value === 'undefined')) {
	...
	$.fn.fileUploadUIX = function(method) {
		if (methods[method]) {
			...
			$.error('Method "' + method
					+ '" does not exist on jQuery.fileUploadUIX');
		}
	};
