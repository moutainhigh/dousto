...
function orderSearch(){
	...
	form.action="sms_virtual_query.action";
	$('#pageNumber').val(0);
	form.submit();
}


function sendSmsVirtual(object,itemNo){
    var form = document.getElementById("sendForm");
	form.action="sms_send_virtual.action";
	$('#itemNo').val(itemNo);
	form.submit();
}
</script>
...
<body class="list">
	<div class="body">
		...
		<form id="listForm" action="sms_virtual_query.action" method="post" >
			...
			<table class="listTable"  style="border: 1px solid #CCCCCC;width:100%">
				...
				<#list pager.list as list>
					<tr>
						...
						<td width="15%">主订单号：<font color="#008B8B">${list.orderNo}</font> <br/>
						...
						<td width="15%">收件人：<font color="#104E8B">${(list.userName)!}</font><br/>
						...
						<td width="15%">
							<#if (list.orderItemVirtuals)??>
								<#list list.orderItemVirtuals as item>
									<#if (item.orderItemClass)??>
									手机号:${(item.receiveMobile)!}
									<span style="width:260px; display:inline-block;line-height:20px;">
									<font color="#104E8B">${(item.orderItemClass)!}</font> 
									</span>
									<input type="button" value="发送短信" onclick="sendSmsVirtual(this,'${(item.orderItemNo)!}');"/>
									</#if>
								</#list>
							</#if>
						</td>
