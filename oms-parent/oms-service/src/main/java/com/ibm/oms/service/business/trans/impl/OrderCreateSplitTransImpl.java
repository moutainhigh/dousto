 ...
 */
@Service("orderCreateSplitTrans")
public class OrderCreateSplitTransImpl extends OrderCreateTransAbstract {
    ...
    protected OrderMain saveOrderMain(OrderMainDTO omDTO, ContextBtcOmsReceiveDTO context) {
    	 OrderMain om = new OrderMain();
	        ...
	        //++++++++++++　订单创建-拆分单以后变成-待审核状态＋＋＋＋＋＋
	        om.setStatusTotal(omDTO.getStatusTotal());
	        om.setStatusPay(omDTO.getStatusPay());
	        om.setStatusConfirm(omDTO.getStatusConfirm());
	      //++++++++++++　订单创建-拆分单以后变成-待审核状态＋＋＋＋＋＋end
	        try {
...
//	          om.setDiscountTotal(discountTotal);
	            ...
	            logger.error(getErrorMsgMain(), e);
	            throw new RuntimeException(getErrorMsgMain() + e);
	        }
	        save(om);
