        ...
        try {
            if(cancelSceneEnum.equals(CancelOrderConst.CancelOrder_Scene_VALIDATED)) { // 库存已扣减未出库取消
             ...
             //   this.orderCreateService.inventoryUnLock(orderMainList); //此接口没有返回信息，故停用
                ...
                InventoryLockOutputDTO output2 = orderCreateService.inventoryLock(orderMain.getOrderNo());
...
//            OrderSub targetOrderSub = orderSub;
    ...
    public ResultDTO updateApproveSaleAfterOrder(String orderSubNoR){
        ...
        if(updateIntegralFlag && (OrderMainConst.OrderMain_OrderCategory_Return.getCode().equals(orderMainR.getOrderCategory()) || OrderMainConst.OrderMain_OrderCategory_Refund.getCode().equals(orderMainR.getOrderCategory()))){
        ...
        //支付掉的积分
        if(updateIntegralPlusFlag && (OrderMainConst.OrderMain_OrderCategory_Return.getCode().equals(orderMainR.getOrderCategory()) || OrderMainConst.OrderMain_OrderCategory_Refund.getCode().equals(orderMainR.getOrderCategory()))){
...
//        boolean tmsResult = returnChangeOrderService.sendOmsToTmsSaleAfterOrder(orderSubNoR);
        ...
        }else if(OrderMainConst.OrderMain_OrderCategory_Reject.getCode().equals(orderCategory))
        {
        ...
        }else if(OrderMainConst.OrderMain_OrderCategory_Refund.getCode().equals(orderCategory)){//退款单
        	//1、扣减积分：需退款，且为“退货”(换货和拒收不扣减积分)，则需在审核时扣除积分[调用会员接口]
		    ...
		    boolean updateStatusFlag2 = orderStatusService.saveProcess(orderSubNoR,OrderStatusAction.S027080, userName, new Date(), null);    
    ...
    public ResultDTO updateCancelSaleAfterOrder(String orderSubNoR){
        ...
        if(statusConfirm.equals(OrderStatus.StatusTypeCode_refundOrder_StatusConfirm_Pass.getCode())){ 
            ...
            if(statusTotal.equals(OrderStatus.RET_ORDER_ON_THE_WAY.getCode()) || statusTotal.equals(OrderStatus.RET_ORDER_REFUNDING.getCode())){
                result.setErrorMessage("已收货的逆向订单不能取消");
                return result;
            }
...
//                        CommonConst.OrderStatusSyncLog_SyncScene_Pay.getCode());
    ...
    public ResultDTO updateCancelConfirm(String orderSubNoR){
        ...
        if(statusConfirm.equals(OrderStatus.StatusTypeCode_refundOrder_StatusConfirm_Pass.getCode())){ 
        	//已审核
        	//orderMainR.setStatusConfirm(OrderStatus.StatusTypeCode_refundOrder_StatusConfirm_Wait.getCode());
        	//orderMainService.save(orderMainR);
            ...
            /*if(OrderMainConst.OrderMain_OrderCategory_Refund.getCode().equals(orderMainR.getOrderCategory())){//退款单
