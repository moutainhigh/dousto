...
public class MiyaOnlinePayServiceImpl implements OnlinePayService{
	...
	public String sendImprestPay(String jsonObj) throws Exception {
		PayJsonDto payJsonDto = JSON.parseObject(jsonObj, PayJsonDto.class);//输入参数
		...
		if((null != payJsonDto.getOrderNo()&&!"".equals(payJsonDto.getOrderNo())) && (null != payJsonDto.getPayType() && !"".equals(payJsonDto.getPayType()))){
			...
			SortedMap<Object,Object> tlvmap = new TreeMap<Object,Object>();  
			 ...
			 tlvmap.put("B3", orderInfo.getB3());//手机小票标题 顾客手机小票显示的标题
			 tlvmap.put("B4", orderInfo.getB4());//金额 单位分，1 分为 1,1 元为 100 
			 tlvmap.put("B5", orderInfo.getB5());//商品信息 商品信息
//			 tlvmap.put("B13",CommonUtilService.callbackMiyaUrl);//通知地址 接收异步通知回调地址
			...
			return JaxbUtil.payResponse(returnStr);
		}
		return JaxbUtil.payResponse(returnStr);
	}
	/**
	...
	public String sendCreatePay(String jsonObj) throws Exception {
		...
		if((null!=payJsonDto.getOrderNo()&&!"".equals(payJsonDto.getOrderNo())) && (null != payJsonDto.getPayType()&&!"".equals(payJsonDto.getPayType()))){
			...
			SortedMap<Object,Object> tlvmap = new TreeMap<Object,Object>();  
			 ...
			 tlvmap.put("B4", orderInfo.getB4());//金额 单位分，1 分为 1,1 元为 100 
			 tlvmap.put("B5", orderInfo.getB5());//商品信息 商品信息
			 tlvmap.put("B11", orderInfo.getB11());//openid/userid 微信 openid 或支付宝 userid。支付宝必填， 微信 openid 和 sub_openid 不能同时为空， JSAPI、WXA 必传
			 tlvmap.put("B12", orderInfo.getB12());//sub_openid  微信 sub_openid, JSAPI、WXA 必传 
			 tlvmap.put("B13", CommonUtilService.getPayCallbackUrl());//通知地址  接收异步通知回调地址 
			 tlvmap.put("B14", orderInfo.getB14());//用户 ip 用户终端 ip。微信 H5、APP 支付必填 
			 tlvmap.put("B16", orderInfo.getB16());//appid  商户微信 APPID
			 tlvmap.put("B17", orderInfo.getB17());//start_time 订单开始时间，格式 yyyy-MM-dd HH:mm:ss 
			 tlvmap.put("B18", orderInfo.getB18());//expire 订单过期时间，格式 yyyy-MM-dd HH:mm:ss
			 String sign = MiyaPayUtil.newCreateSign(keyValue, tlvmap);//生成签名
			 tlvmap.put("A8", sign);//设置签名值 
