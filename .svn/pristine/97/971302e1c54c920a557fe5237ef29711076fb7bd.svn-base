...
import com.ibm.oms.service.pay.dto.PayOnLineCDto;
...
public class NewSaleAfterOrderServiceImpl implements NewSaleAfterOrderService {
	...
	private OrderMain setRetOrderMain(String applySource,String orderCategory,ReturnHeaderOrderFromLasaDto headerData,OrderMain orderOrg){
		...
		if(StringUtils.isNotBlank(headerData.getReturnShopNo())){
			...
			retOrder.setShipStoreCode(orderOrg.getMerchantNo());//商家编号
		}
...
//		retOrder.setTotalProductPrice(totalProductPrice);
		...
		if(StringUtils.isNotBlank(headerData.getReturnPoint())){
			retOrder.setTotalPointAmount(new BigDecimal(headerData.getReturnPoint()));
		}
		// 优惠金额总计
...
//						retItem.setReason(itemDto.);//退换货原因
//						retItem.setRemark();
//						retItem.setRefOrderItemId();
            ...
            if(null!=orderItem){
               ...
               /* if(StringUtils.isEmpty(item.getStockNo())){
                ...
                if(null==item.getPayAmount() || item.getPayAmount().compareTo(new BigDecimal(0))==0){
                    item.setPayAmount(orderItem.getUnitPrice().multiply(new BigDecimal(item.getSaleNum())));
                }
...
//            this.orderPayService.saveChangePaidByOrderMain(orderMain);
            ...
            //库存扣减
            /*MessageDto msgDto = changeStock("subtract","4",orderNo,orderMain.getShipStoreCode(),items);            
            if(msgDto != null && msgDto.isSuccess()){
...
//            logger.info(orderSub.getIdOrder()+"");
	...
	public PayOnLineCDto refundPay(){
		try{			
			PayOnLineADto payA = new PayOnLineADto();
			PayOnLineBDto payB = new PayOnLineBDto();
			...
			PayOnLineCDto payC = JaxbUtil.converyToJavaBean(returnStr,PayOnLineCDto.class);
			return payC;
		}catch(Exception e){
			/*resultDTO.setErrorMessage(String.format("approveRefundOrder 退款失败, 退款单号:%s", orderNo));
            return resultDTO;*/
		}
