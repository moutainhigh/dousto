...
$().ready( function() {
	var activeTabMapping = {1:0, 4:1, 10:2, 8:3, 7:4};
	$( "#tabs" ).tabs({
		...
		<div id="conditions" style="margin:0;padding:0;">
			<table class="listTable" style="border: 1px solid #e6e6e6; background: #FFFFFF;line-height: 30px;">
				...
				<tr>
					...
					<td colspan="1">支付方式：
			      		<input type="hidden" id="payModeCode"  value="${orderPayMode.payModeCode}"/>
			      		<select name="orderPayMode.payModeCode"  id="payWay" onchange="getCodPayWay()">
							<#list paymentModeList as list>
								<option value="${list.code}"<#if list.code == orderPayMode.payModeCode> selected </#if>>
									<#if list.code == "">
									全部
									<#else>
										${list.name}
				...
				<tr>
					...
					<td>买家留言<select id="isClientRemark" name="includeOrNot.isClientRemark">
				...
				<tr>
					<td>商家备注：<select id="isRemark" name="includeOrNot.isRemark">
						...
						<input type="text" id="remark" name="orderMain.remark" value="${orderMain.remark!}" /></td>
					<td colspan='2'>客服备注：
			...
			<table class="listTable detailTable"  style="border: 1px solid #CCCCCC;width:2200px;>
				...
				<tr>
					...
					<th width="70px">
						<span class="sort" name="skuNumber">sku种类数量</span>
					</th>
					...
					<th width="170px">
						<span class="sort" name="sellerMessage">商家备注</span>
					</th>
					<th width="170px">
						<span class="sort" name="remark">订单备注</span>
					</th>
				...
				<#list pager.list as list>
					<tr ondblclick="window.open('order!view.action?orderNo=${list.orderNo}&column=${column}')">
						...
						<td>
							...
							<#assign y = 0> 
								<#list list.orderItems as item>
									<#if (item.commodityCode!=y)>
									
							<#assign x = x+1> 
								
							<#assign y = item.commodityCode> 
									</#if>
								</#list>
							</#if>
								&nbsp;&nbsp;&nbsp;&nbsp;  <font color="#CD661D">${(x)!}</font> &nbsp;&nbsp;&nbsp;&nbsp; <br/>
						...
						<td>
							客户留言：<font color="#CD661D">${(list.clientRemark)!}</font> <br/>
							</td>
						<td>
							商家备注：<font color="#CD661D">${(list.sellerMessage)!}</font> <br/>
							</td>
						<td>
							订单备注：<font color="#CD661D">${(list.remark)!}</font> <br/>
							</td>
							<td>
							<#if (list.orderItems)??>
								<#list list.orderItems as item>
						...
						<td>
							单号：<font color="#104E8B">${(list.logisticsOutNo)!}</font><br/>	
									...
									<td>
							<#if (list.payModeName)??>
							      		${list.payModeName} : <font color="red">${(list.payAmount!0)?string.currency}</font> <br/>
							      		<input type="hidden" id="payNameTmp" value="${list.payModeName}"/>
							      		<input type="hidden" id="payAmountTm p" value="${(list.payModeAmount!0)}"/>
							      		</#if>
						</td>
