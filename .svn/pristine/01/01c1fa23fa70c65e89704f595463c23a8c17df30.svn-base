...
//    			headData.setReturnReason(o.get);//退货原因
    			...
    			StatusDict confirmDict =  orderStatusMap.get(o.getStatusTotal());
    			if (null != confirmDict) {
    				headData.setReturnStatusName(confirmDict.getStatusName());//退货单状态名称
    			}
    	...
    	try{
    		...
    		if(itemList != null && itemList.size() > 0){
    			for(ReturnItemOrderFromLasaDto itemDto : itemList){
    				...
    				if(headDto != null){
    					product.setHandleType(headDto.getReturnType());//处理方式
    					product.setApplyReason(headDto.getReturnReason());//申请原因
    				}
