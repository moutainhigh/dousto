...
@Service("saleAfterTransService")
public class SaleAfterTransServiceImpl implements SaleAfterTransService {
    ...
    @Autowired
    NewSaleAfterOrderService newSaleAfterOrderService;
    @Autowired
...
//            OrderSub targetOrderSub = orderSub;
    ...
    public ResultDTO updateApproveSaleAfterOrder(String orderSubNoR){
        ...
        // 设置审核人名称
        String userName = userUtil.getLoginUserRealName();
        orderMainR.setConfirmerName(userName);        
        ...
        if(OrderMainConst.OrderMain_OrderCategory_Change.getCode().equals(orderCategory)){
            ...
            if(null!=chgOutOrderMain){
                ...
                if(OrderStatus.ORDER_AUDITING_MANUAL.getCode().equals(statusTotal) || OrderStatus.ORDER_VALIDATED.getCode().equals(statusTotal)
                        || OrderStatus.ORDER_SENT.getCode().equals(statusTotal) || OrderStatus.ORDER_POD_SENT.getCode().equals(statusTotal) || OrderStatus.ORDER_PAID_CANCEL.getCode().equals(statusTotal)){
...
//        boolean tmsResult = returnChangeOrderService.sendOmsToTmsSaleAfterOrder(orderSubNoR);
        //4、更新审核状态
        boolean approveFlag = orderStatusService.saveProcess(orderSubNoR, OrderStatusAction.S022031, userName, new Date(), null);
        //5、非门店代退，直接进入质检
        orderStatusService.saveProcess(orderSubNoR, OrderStatusAction.S023140, userName, new Date(), null);
        //6、 如果是换货，审核换货出库单
        if(OrderMainConst.OrderMain_OrderCategory_Change.getCode().equals(orderCategory)){
            try {
                String auditResult = manualAudit(orderMainR.getChgOurOrderNo(), true);
                if(StringUtils.isNotBlank(auditResult)){
                	result.setErrorMessage(auditResult);
                }
            } catch (Exception e) {
                logger.info("{}", e);
	        ...
	        boolean updateIntegralFlag = false;// 是否需更新积分（扣减赠送的积分）
	        boolean updateIntegralPlusFlag = false;//是否需要加回积分(支付掉的积分)
	        if (commonUtilService.longToBoolean(orderMainR.getIfNeedRefund())) { // 是否需退款
	            ...
	            BigDecimal payPoint = orderMainR.getTotalPoint();
	            if (payPoint!=null && payPoint.compareTo(new BigDecimal(0)) > 0) {
	            	updateIntegralPlusFlag = true;
	            }            
	        }
        	//会员积分不够扣减时 如何处理？
	        //1、扣减积分：需退款，且为“退货”(换货和拒收不扣减积分)，则需在审核时扣除积分[调用会员接口] 0：加积分，1：减积分
            if(updateIntegralFlag){
                ResultBean bean = pointsTransactionDetailHsService.updateMemberPointsDetail(orderMainR.getMemberNo(), orderMainR.getOrderNo(), "", "1", orderMainR.getTotalGivePoints().intValue(), "2"); //1:支付 2：退款 3：积分兑换          
                if(bean.getCode().equals(CommonConstService.FAILED_CODE)){
                	result.setErrorMessage(bean.getMsg());
                	result.setResultObj(bean.getData());//可退积分
                    return result;
                }
            }
            //支付掉的积分
            if(updateIntegralPlusFlag){
                ResultBean bean = pointsTransactionDetailHsService.updateMemberPointsDetail(orderMainR.getMemberNo(), orderMainR.getOrderNo(), "", "0", orderMainR.getTotalPoint().intValue(), "2");           
                if(bean.getCode().equals(CommonConstService.FAILED_CODE)){
                	result.setErrorMessage(bean.getMsg());
                	result.setResultObj(bean.getData());//可退积分
                    return result;
            ...
            if(OrderMainConst.OrderMain_OrderCategory_Refund.getCode().equals(orderCategory)){//退款单
            	//退货审核中-->退款中（退款单） 
	        	boolean finishFlag = false;//完成标识    		
			    boolean updateStatusFlag2 = orderStatusService.saveProcess(orderSubNoR,OrderStatusAction.S022070, userName, new Date(), null);    
			    if(updateStatusFlag2){			        
			    	//调用退款接口
			    	//result = newSaleAfterOrderService.refundPay(orderMainR);
			    }	    		
	    		boolean updateStatusFlag3 = orderStatusService.saveProcess(orderSubNoR,OrderStatusAction.S027080, userName, new Date(), null);    
	    	    if(updateStatusFlag3){
	    	        finishFlag = true;
	    	    }
	    	    logger.info("审核:"+orderMainR.getOrderNo()+",结束标识:"+finishFlag);
    ...
    public String manualAudit(String orderNo, boolean approved) {
        ...
        if (approved) {
            ...
            //扣减库存
            MessageDto msgDto = newSaleAfterOrderService.changeStock("subtract","4",orderNo,ClientConstant.ESTORE_WAREHOUSE_CODE,null,om.getOrderItems());
            if(msgDto != null){
	            boolean inventorySucceed = msgDto.isSuccess();
	            if (inventorySucceed) {
	               
	            }else{
	            	return msgDto.getMsg();
	            }
            }else{
            	return "扣减库存失败！";
            }
    ...
    public ResultDTO updateUnConfirmSaleAfterOrder(String orderSubNoR){
        ...
        if(statusConfirm.equals(OrderStatus.StatusTypeCode_refundOrder_StatusConfirm_Wait.getCode())){
            //当前审核状态：未审核
            try {
            ...
            if(OrderMainConst.OrderMain_OrderCategory_Change.getCode().equals(orderCategory))
            {
                ...
                try {
                    orderStatusService.updateStatus(chgOutSubNo, OrderStatusAction.S014253, null, null, null);
                } catch (Exception e) {
                    logger.info("{}", e);
                    result.setErrorMessage(""+e.getMessage());
                }
    ...
    public ResultDTO updateCancelSaleAfterOrder(String orderSubNoR){
        ...
        if(statusConfirm.equals(OrderStatus.StatusTypeCode_refundOrder_StatusConfirm_Pass.getCode())){//已审核
            //已收货的意向单不能取消
            if(statusTotal.equals(OrderStatus.RET_ORDER_ON_THE_WAY.getCode()) || statusTotal.equals(OrderStatus.RET_ORDER_REFUNDING.getCode())){
                result.setErrorMessage("已收货的逆向订单不能置无效");
                return result;
            }
            ...
            if(OrderMainConst.OrderMain_OrderCategory_Change.getCode().equals(orderCategory)){
                ...
                if(null!=chgOutOrderMain){
                    ...
                    if(outStatusTotal.equals(OrderStatus.ORDER_SENT.getCode()) || outStatusTotal.equals(OrderStatus.ORDER_POD_SENT.getCode())){
                        result.setErrorMessage("当前换货单所产生的出库订单："+chgOutSubNo+"已出库，不能置无效");
                        return result;
                    }
                }
            }
            ...
            //2、更改当前审核状态：已审核
            boolean statusResult = orderStatusService.updateStatus(orderSubNoR, OrderStatusAction.S021079, userName, new Date(), null);
            if(!statusResult){
            ...
            //当前审核状态：未审核
            try {
                orderStatusService.updateStatus(orderSubNoR, OrderStatusAction.S021079, null, null, null);
            } catch (Exception e) {
                logger.info("{}", e);
                result.setErrorMessage(""+e.getMessage());
            }   
	...
	public void saveSaleAfterOrderToSAP(String orderSubNoR){
	    try{
	        ...
	        logger.info("意向单 "+orderSubNoR+"同步SAP异常：{}", e);
	    }
	}
    ...
    public ResultDTO updateCancelConfirm(String orderSubNoR){
        ...
        if(statusConfirm.equals(OrderStatus.StatusTypeCode_refundOrder_StatusConfirm_Pass.getCode())){//已审核
            //2、更改当前审核状态：已审核        	
        	boolean statusResult = orderStatusService.updateStatus(orderSubNoR, OrderStatusAction.S021020, userName, new Date(), null);
            if(!statusResult){
                result.setErrorMessage("更新状态失败!");
                return result;
            }     
            ...
            //4、补回积分：需退款，（换货和拒收审核时不扣减积分，无需补回），则需将积分还给顾客（因为审核时已将积分扣除）【不影响主流程】
            if(updateIntegralFlag){
                ResultBean bean = pointsTransactionDetailHsService.updateMemberPointsDetail(orderMainR.getMemberNo(), orderMainR.getOrderNo(), "", "+", orderMainR.getTotalGivePoints().intValue(), "2"); //1:支付 2：退款 3：积分兑换          
                if(bean.getCode().equals(CommonConstService.FAILED_CODE)){
                	result.setErrorMessage(bean.getMsg());
                	result.setResultObj(bean.getData());//可退积分
                    return result;
            ...
            //支付掉的积分
            if(updateIntegralPlusFlag){
                ResultBean bean = pointsTransactionDetailHsService.updateMemberPointsDetail(orderMainR.getMemberNo(), orderMainR.getOrderNo(), "", "-", orderMainR.getTotalPoint().intValue(), "2");           
                if(bean.getCode().equals(CommonConstService.FAILED_CODE)){
                	result.setErrorMessage(bean.getMsg());
                	result.setResultObj(bean.getData());//可退积分
                    return result;
            ...
            //如果是换货，取消换货出库单
            if(OrderMainConst.OrderMain_OrderCategory_Change.getCode().equals(orderMainR.getOrderCategory())){
            	String chgOutOrderNo = orderMainR.getChgOurOrderNo();//换货出库单单号
            	OrderMain chgOutOrderMain = this.orderMainService.getByField(OrderMain_.orderNo,chgOutOrderNo);
                String chgOutSubNo = this.orderNoService.getDefaultOrderSubNoByOrderNo(chgOutOrderNo);
            	if(null!=chgOutOrderMain){
    ...
    public ResultDTO updateFastStorage(String orderSubNoR){
        ...
        if(statusConfirm.equals(OrderStatus.StatusTypeCode_refundOrder_StatusConfirm_Pass.getCode())){
            //2、更改当前审核状态：已审核
        	orderStatusService.saveProcess(orderSubNoR, OrderStatusAction.S024050, userName, new Date(), null);
        	String orderCategory = orderMainR.getOrderCategory();
	        if(OrderMainConst.OrderMain_OrderCategory_Change.getCode().equals(orderCategory)){
	        	boolean statusResult = orderStatusService.updateStatus(orderSubNoR, OrderStatusAction.S025080, userName, new Date(), null);
	            if(!statusResult){
	                result.setErrorMessage("更新状态失败!");
	                return result;
	            }	        	
	        }else{
	            boolean statusResult = orderStatusService.updateStatus(orderSubNoR, OrderStatusAction.S025070, userName, new Date(), null);
	            if(!statusResult){
	                result.setErrorMessage("更新状态失败!");
	                return result;
	            }
