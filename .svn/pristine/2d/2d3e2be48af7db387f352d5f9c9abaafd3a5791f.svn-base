...
public class SubbmitOrderServiceImpl extends SubbmitOrderCommon   implements SubbmitOrderService,SubbmitOrderConfigService<ReceiveOrderMainDTO>{
			...
			for(InfCartItem cartItem : cartItemList){
				...
				if(ri.getSkuId().equals(String.valueOf(cartItem.getSkuId()))){
					...
					if("SINGLE".equals(cartItem.getItemType())){
						...
						//payAmount	行项目应付均摊金额	= 行优惠总额 - 均摊得积分总额 ？
						...
						if(CollectionUtils.isNotEmpty(ri.getBundleItemList())){
							...
							for(ReceiveOrderItemDTO iex : itemExt){
								...
								if("GIFT".equals(iex.getItemType())){
									...
									//查询赠品信息
									//oidex.setOrderItemClass(OrderItemConst.OrderItem_OrderItemClass_NORM.getCode());
									//oidex.setHasGift(OrderItemConst.OrderItem_OrderItemClass_GIFT_NO.getCode());
									//items.add(oidex);
								}
							}
					...
					if("GIFT".equals(cartItem.getItemType())){
						//计算订单行信息
						OrderItemDTO oid =  new OrderItemDTO();
						oid.setHasGift(Integer.valueOf(OrderMainConst.OrderItem_OrderItemClass_GIFT_YES.getCode()));
					...
					if("BUNDLE".equals(cartItem.getItemType())){
						//oid.setHasGift(OrderMainConst.OrderItem_OrderItemClass_GIFT_NO.getCode());
					}
	 ...
	 * @return
	 */
	private String calculateTotalPayAmount(String totalOrderAmount, String transportFee, String discountTransport) {
	...
	private String calculateTotalOrderAmount(InfCart calculateAftercart,String pointDiscountAmount) {
		...
		return commonUtilService.bigDecimal2String(totalOrderAmount);
	}
...
//		}
	...
	private OrderMainDTO buildBtcOrderMain(ReceiveOrderMainDTO ro, InfCart calculateAftercart,OrderMainDTO omd) {
		...
		//订单实际应付金额总计 = 订单金额总计 - 运费总额-运费优惠
		String totalPayAmount = calculateTotalPayAmount(totalOrderAmount,transportFee,discountTransport);
		...
		List<OrderPayDTO> orderPayDTOs = buildOrderPay(omd);
	...
	private List<OrderPayDTO> buildOrderPay(OrderMainDTO omd) {
		List<OrderPayDTO> pays = new ArrayList<OrderPayDTO>();
		//优惠卷支付 现阶段没有orderId和orderNo待校验成功订单保存成功后赋值
		if(StringUtils.isNotEmpty(omd.getTotalPromo())){
		...
		if(StringUtils.isNotEmpty(omd.getTotalPointAmount())){
			OrderPayDTO op =  new OrderPayDTO();
		...
		if(StringUtils.isNotEmpty(omd.getTotalPoint())){
			OrderPayDTO op =  new OrderPayDTO();
